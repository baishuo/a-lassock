...
public class TextHill extends AbstractHill<TextSign, TextShade> {
                            ...
                            synchronized (courseLock) {
                                ...
                                if (newPath == null) {
                                    for (Course course : currCourses) {
                                        ...
                                        FilePathParseInfo filePathParseInfo = DirectorScannerUtils.parseFilePath(course
                                        ...
                                        for (String filePath : fileList) {
                                            File file1 = new File(filePathParseInfo.getBasePath(), filePath);
    ...
    protected List<TextSign> makeDetails(Course course) {
        ...
        String fileIncludesJson = course.getString(CourseConst.text.FILES_INCLUDES);
        String filesExcludesJson = course.getString(CourseConst.text.FILES_EXCLUDES);
        FilePathParseInfo filePathParseInfo = DirectorScannerUtils.parseFilePath(course.getString(CourseConst.text.DATA_INPUT_PATH));
        String[] fileList = DirectorScannerUtils.scannerFiles(fileIncludesJson, filesExcludesJson, filePathParseInfo);
        for (String filePath : fileList) {
            File file = new File(filePathParseInfo.getBasePath(), filePath);
    ...
    protected boolean compareCourse(Course newc, Course old) {
        ...
        if (!newc.getString(CourseConst.text.FILES_EXCLUDES).equals(
                old.getString(CourseConst.text.FILES_EXCLUDES))) {
            return false;
        }
        if (!newc.getString(CourseConst.text.FILES_INCLUDES).equals(
                old.getString(CourseConst.text.FILES_INCLUDES))) {
            return false;
        }
